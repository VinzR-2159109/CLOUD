...
<head>
    ...
    <title>Running History Service Forms</title>
    <link rel="stylesheet" href="{{ asset('css/runningHistoryForm.css') }}">
</head>
<body>

    <h1>Running History Service Forms</h1>
    ...
    @csrf
        ...
        <button type="submit">Get Running History</button>
    </form>

    <form action="{{ url('/calculate-average-pace') }}" method="post">
    @csrf
        <label for="calculatePaceUserId">User ID:</label>
        <input type="text" id="calculatePaceUserId" name="userId" required>
        <button type="submit">Calculate Average Pace</button>
    </form>
